...
def apply_gravity(gravitational_coefficient, moon_vehicle):
    vector = moon_vehicle.multiply(gravitational_coefficient / pow(moon_vehicle.get_module(), 3))
    return vector


# NOTE that vehicle orientation vector's module must be equal to one
...
def apply_resistance(vehicle_mass, atmospheric_density, resistance_coefficient, vehicle_resistance_area, vehicle_speed):
    ...
    vector = vehicle_speed.multiply(constant * vehicle_speed.get_module(), )
    return vector
